#!/usr/bin/env python3
"""
MCP Server for an LLM Knowledge Wiki.
...
# ─────────────────────────────────────────────────────────────
# MCP import is deferred to runtime (see _require_mcp) so this module can be
# imported for its WikiIndex/parsing helpers (e.g. by wiki_ui.py) without the
# `mcp` package installed.
# ─────────────────────────────────────────────────────────────


def _require_mcp():
    """Import the MCP SDK, exiting with install instructions if missing."""
    try:
...
logger = logging.getLogger("llm-wiki-mcp")
...
def create_server(wiki_dir: Path, raw_dir: Path, allow_ingest: bool = False) -> "Server":
    ...
    Server, _stdio, TextContent, Tool = _require_mcp()
    ...
    async def list_tools() -> list[Tool]:
        tools = [
            Tool(
                name="wiki_search",
                description=(
                    "Search the wiki for pages matching a query. Returns ranked results "
                ...
                inputSchema={
                    ...
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": "Search query — use the wiki's domain terms (e.g., 'a key concept', 'an entity name', 'a topic phrase')",
                        },
                        "page_type": {
                            "type": "string",
                            "description": "Optional filter: entity, concept, topic, comparison, open-question",
                            "enum": ["entity", "concept", "topic", "comparison", "open-question"],
                        },
                        "tags": {
                            "type": "array",
                            "items": {"type": "string"},
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "page_name": {
                            "type": "string",
                            "description": "Page name (filename without .md extension, e.g., 'a-page-name')",
                        },
                    },
                    "required": ["page_name"],
                },
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "tag": {
                            "type": "string",
                            "description": "Optional filter by tag (e.g., 'a-tag', 'another-tag')",
                        },
                    },
                },
            ),
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="MCP server for the LLM Knowledge Wiki")
    ...
    _server_cls, stdio_server, _tc, _tool = _require_mcp()
    server = create_server(wiki_dir, raw_dir, allow_ingest=args.allow_ingest)

    logger.info(f"Starting LLM Wiki MCP server (wiki: {wiki_dir}, ingest: {args.allow_ingest})")
