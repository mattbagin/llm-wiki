...
def cmd_ingest_approved(args: argparse.Namespace) -> int:
    ...
    # research.py lives alongside this package in the deployed wiki's tools/ dir.
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
    try:
        import research
    except ImportError as e:
        print(f"could not import research.py (expected in tools/): {e}", file=sys.stderr)
        return 1

    queue = ApprovalQueue(Path(args.inbox))
    wiki_root = Path(args.wiki).resolve().parent  # wiki/ -> repo root
    archive_dir = queue.root / "ingested"
    archive_dir.mkdir(parents=True, exist_ok=True)

    count = 0
    for item in list(queue.iter_approved()):
        ...
        if not item.content_path.exists():
            print(f"skipping {item.item_id}: no content file", file=sys.stderr)
            continue

        print(f"ingesting {item.item_id} ...")
        ...
        # Archive so it isn't re-ingested next run.
        for p in (item.evaluation_path, item.content_path):
            if p.exists():
                p.rename(archive_dir / p.name)
        count += 1

    print(f"ingested {count} approved item(s); drafts printed above for review before saving to wiki/")
    return 0
