...
Checks wiki health across multiple dimensions:
  ...
  9. Verbatim sources — protected (verbatim: true) sources flagged for review
...
def check_verbatim_sources(raw_dir: Path, base: Path) -> list[LintIssue]:
    """Surface protected (`verbatim: true`) sources so the human can confirm wiki
    reproductions quote them exactly rather than paraphrasing them."""
    issues: list[LintIssue] = []
    if not raw_dir.exists():
        return issues
    for md_file in raw_dir.rglob("*.md"):
        fm = parse_frontmatter(md_file.read_text(encoding="utf-8", errors="replace"))
        if str(fm.get("verbatim", "")).lower() == "true":
            issues.append(
                LintIssue(
                    check="verbatim-source",
                    severity="info",
                    file=relative_path(md_file, base),
                    message="Protected source (verbatim: true) — its text must not be paraphrased",
                    suggestion="Confirm wiki pages reproduce it only inside `> [!quote] Verbatim` blocks "
                    "and keep analysis on separate pages",
                )
            )
    return issues
...
def main() -> None:
    # Ensure Unicode icons render on Windows consoles (cp1252) without crashing.
    try:
    ...
    report.issues.extend(check_verbatim_sources(raw_dir, base))
